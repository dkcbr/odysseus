...
if ! getent passwd "$PUID" >/dev/null 2>&1; then
    # ⭐ FIX: Do NOT use /app as home directory
    useradd -u "$PUID" -g "$PGID" -m -s /bin/sh odysseus
fi

ODY_USER="$(getent passwd "$PUID" | cut -d: -f1)"
[ -z "$ODY_USER" ] && ODY_USER=odysseus

# Docker socket group plumbing
...
is_broad_mount_root() {
    case "$1" in
        /|/home|/srv|/var|/usr|/opt|/tmp|/mnt|/media) return 0 ;;
    esac
    return 1
}
...
repair_app_tree_ownership() {
    if [ -d /app ]; then
        find /app -xdev \
            \( -path /app/data -o -path /app/logs -o -path /app/.ssh -o -path /app/.cache -o -path /app/.local -o -path /app/_builtin_mcp_src \) -prune \
